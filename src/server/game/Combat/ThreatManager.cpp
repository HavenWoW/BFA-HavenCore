...
void ThreatManager::AddThreat(Unit* target, float amount, SpellInfo const* spell, bool ignoreModifiers, bool ignoreRedirects)
{
    ...
    // If victim is a personal spawn, redirect all aggro to its summoner. 8.3.7 has no
    // generic private-object owner, so the check goes through TempSummon visibility.
    if (TempSummon* tempSummonVictim = target->ToTempSummon())
    {
        if (tempSummonVictim->IsVisibleBySummonerOnly())
        {
            // Personal spawns sharing a summoner can still aggro each other
            TempSummon* tempSummonOwner = GetOwner()->ToTempSummon();
            if (!tempSummonOwner || !tempSummonOwner->IsVisibleBySummonerOnly() ||
                tempSummonVictim->GetSummonerGUID() != tempSummonOwner->GetSummonerGUID())
            {
                if (Unit* summoner = tempSummonVictim->GetSummoner())
                {
                    AddThreat(summoner, amount, spell, ignoreModifiers, ignoreRedirects);
                    amount = 0.0f;
                }
            }
        }
