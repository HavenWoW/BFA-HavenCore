    ...
    switch (e.GetActionType())
    {
        ...
        case SMART_ACTION_FORCE_COMPLETE_QUEST:
        {
            for (WorldObject* const target : targets)
            {
                if (IsPlayer(target))
                {
                    target->ToPlayer()->ForceCompleteQuest(e.action.quest.quest);
        ...
        case SMART_ACTION_CIRCLE_PATH:
        {
            for (WorldObject* target : targets)
            {
                ...
                me->GetMotionMaster()->MoveCirclePath(target->ToUnit()->GetPositionX(), target->ToUnit()->GetPositionY(), target->ToUnit()->GetPositionZ(), (float)e.action.moveCirclePath.radius, e.action.moveCirclePath.clockWise, uint8(e.action.moveCirclePath.stepCount));
            }
